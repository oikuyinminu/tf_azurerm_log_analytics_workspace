locals {
  log_analytics_workspace_name = upper(format("%s-%s-%s-LAW-%s-%03d", var.type, local.region[lower(var.location)], local.env[lower(local.tags["Environment"])], var.instance))
